...
import { APIPromise } from 'dbahn-stations/core/api-promise';

import util from 'node:util';
import DbahnStations from 'dbahn-stations';
import { APIUserAbortError } from 'dbahn-stations';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['DBAHN_STATIONS_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: DbahnStations) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new DbahnStations({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['DBAHN_STATIONS_LOG'] = 'debug';
      const client = new DbahnStations({
        logger: logger,
        clientID: 'My Client ID',
        clientSecret: 'My Client Secret',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['DBAHN_STATIONS_LOG'] = 'not a log level';
      const client = new DbahnStations({
        logger: logger,
        clientID: 'My Client ID',
        clientSecret: 'My Client Secret',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'DBAHN_STATIONS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['DBAHN_STATIONS_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['DBAHN_STATIONS_BASE_URL'] = 'https://example.com/from_env';
      const client = new DbahnStations({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['DBAHN_STATIONS_BASE_URL'] = ''; // empty
      const client = new DbahnStations({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
      expect(client.baseURL).toEqual('https://apis.deutschebahn.com/db-api-marketplace/apis/station-data/v2');
    });

    test('blank env variable', () => {
      process.env['DBAHN_STATIONS_BASE_URL'] = '  '; // blank
      const client = new DbahnStations({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
      expect(client.baseURL).toEqual('https://apis.deutschebahn.com/db-api-marketplace/apis/station-data/v2');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['DBAHN_STATIONS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new DbahnStations({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    ...
    // default
    const client2 = new DbahnStations({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['DBAHN_STATIONS_CLIENT_ID'] = 'My Client ID';
    process.env['DBAHN_STATIONS_CLIENT_SECRET'] = 'My Client Secret';
    const client = new DbahnStations();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['DBAHN_STATIONS_CLIENT_ID'] = 'another My Client ID';
    process.env['DBAHN_STATIONS_CLIENT_SECRET'] = 'another My Client Secret';
    const client = new DbahnStations({ clientID: 'My Client ID', clientSecret: 'My Client Secret' });
    expect(client.clientID).toBe('My Client ID');
    expect(client.clientSecret).toBe('My Client Secret');
  });
