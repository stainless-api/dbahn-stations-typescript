...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: DbahnStations)`
    : `const run: (${functionSource.client}: DbahnStations) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.stations.retrieve',
    'client.szentralen.list',
    'client.szentralen.retrieve',
  ],
